...
def assign_orientation(kps, octave, num_bins=36):
    ...
    for kp in kps:
        ...
        if kernel.shape[0] <= 2*w or kernel.shape[1] <= 2*w:
            continue
        L = octave[...,s]
    ...
    for kp in kps:
        ...
        for i in range(0, subregion_w):
            for j in range(0, subregion_w):
                ...
                hist = get_histogram_for_subregion(m[t:b, l:r].ravel(), theta[t:b, l:r].ravel(), num_bin, kp[3], bin_width, subregion_w, i)
                featvec[i*subregion_w*num_bin + j*num_bin:i*subregion_w*num_bin + (j+1)*num_bin] = hist.flatten()
        featvec /= max(1e-6, norm(featvec))
...
sft = SIFT(mpimg.imread('lenna50.jpg'))
pts = sft.get_features()
print(pts)