...
def predict(args, model, valid_loader, cuda=False):
    model.load_state_dict(torch.load(os.path.join(args.save_directory, args.checkpoint), map_location={'cuda:0': 'cuda' if cuda else 'cpu'}))  # , strict=False
        ...
        with open(output_file, 'w+') as f:
            for i, batch in enumerate(valid_loader):
                ...
                for r in range(len(outputs)):
                    f.write(' '.join(('%s %s' % (path[r], ' '.join([str(float(rect[i][r])) for i in range(4)])), ' '.join([str(i) for i in outputs[r]])))+'\n')
