...
def predict(args, model, valid_loader):
    model.load_state_dict(torch.load(os.path.join(args.save_directory, args.checkpoint)))  # , strict=False
...
def test(args, model, valid_loader, output_file='output.txt'):
    model.load_state_dict(torch.load(os.path.join(args.save_directory, args.checkpoint)))  # , strict=False
    model.eval()  # prep model for evaluation
    with torch.no_grad():
