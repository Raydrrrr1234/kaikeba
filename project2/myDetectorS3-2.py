...
def train(args, train_loader, valid_loader, model1, model2, criterion1, criterion2, optimizer1, optimizer2, device,
    ...
    for epoch_id in range(1, epoch + 1):
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            # two category
            positive_cat = torch.LongTensor([1, 0]).to(device)
            negative_cat = torch.LongTensor([0, 1]).to(device)
            ...
            # target of detection
            output_face_positive = torch.cat([torch.unsqueeze(output_m1[i], 0) for i in range(batch['face'].shape[0])
                                              if batch['face'][i]], dim=0)
            output_face_negative = torch.cat([torch.unsqueeze(output_m1[i], 0) for i in range(batch['face'].shape[0])
                                              if not batch['face'][i]], dim=0)

            # calculate face detection loss
            loss1 = criterion1(output_face_positive.transpose(1, 0), positive_cat)
            loss1 += criterion1(output_face_negative.transpose(1, 0), negative_cat)

            # construct new dataset for face key point output
            input_img = torch.index_select(input_img, 0,
                                           torch.tensor([1 for i in range(batch['face'].shape[0]) if batch['face'][i]]))
            selected_landmark = torch.index_select(landmark, 0, torch.tensor(
                [1 for i in range(batch['face'].shape[0]) if batch['face'][i]]))
            target_pts = selected_landmark.to(device)
            ...
            loss = loss1 * img.shape[2] + loss2 * output_face_negative.shape[0] / output_face_positive.shape[0]
        ...
        with torch.no_grad():
            ...
            for valid_batch_idx, batch in enumerate(valid_loader):
                ...
                # target of detection
                output_face_positive = torch.cat(
                    [torch.unsqueeze(output_m1[i], 0) for i in range(batch['face'].shape[0])
                     if batch['face'][i]], dim=0)
                output_face_negative = torch.cat(
                    [torch.unsqueeze(output_m1[i], 0) for i in range(batch['face'].shape[0])
                     if not batch['face'][i]], dim=0)
                ...
                # calculate total loss
                loss = loss1 * img.shape[2] + loss2 * (output_face_negative.shape[0] / output_face_positive.shape[0] + 1)
        ...
        if args.save_model and epoch_id % args.save_interval == 0:
            ...
            torch.save(model1.state_dict(), saved_model_name2)
...
def main_test():
    ...
    parser.add_argument('--loss1', type=str, default='L1',
                        help='loss2 function')
    ...
    criterion_pts1 = nn.CrossEntropyLoss()
    if args.loss2 == 'L2':
    ...
    elif args.alg1 == 'adam' or args.alg == 'Adam':
    ...
    elif args.alg2 == 'adam' or args.alg == 'Adam':
